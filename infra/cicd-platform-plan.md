...
- 边界：本仓库当前是 **Quarkus 应用源码**（admin-server）；平台与 GitOps 清单将放到 **两个新仓库**：`infra` + `apps-deploy`。不修改业务逻辑代码。
...
### 3.1 平台组件版本矩阵（建议写入 infra/versions.env）
...
- Nexus 就绪后（强制收敛）：
  ...
  - Tekton 官方 YAML：将 release.yaml（含镜像引用）**下载固化**到 `infra/platform/tekton/upstream/`（并可选记录 sha256），由 Argo CD 纳管，避免运行时直连。
...
> 约定：
> - **所有版本必须锁定**（写在 `infra/versions.env`），setup.sh 只引用版本变量。
> - **所有集群资源来自 Git**：平台组件来自 `infra`；应用资源来自 `apps-deploy`。
...
- [x] B0. 安装 MetalLB（版本锁定）并配置地址池：`192.168.10.100-192.168.10.150`
  - 产物（infra）：`bootstrap/metallb/ipaddresspool.yaml`、`bootstrap/metallb/l2advertisement.yaml`
  - 验收：创建一个 `type: LoadBalancer` Service 可获得该地址池中的 IP。
- [x] B1. 安装 ingress-nginx（版本锁定），Service 类型使用 LoadBalancer（由 MetalLB 分配 IP）
  - 产物（infra）：`bootstrap/ingress-nginx/values.yaml`（或 kustomize overlay）
  - 验收：`ingress-nginx-controller` 获得固定 LB IP；从局域网可访问 80/443。
- [x] B2. 安装 cert-manager（版本锁定）
  - 产物（infra）：`bootstrap/cert-manager/`（helm values + issuer/ca secret）
- [x] B3. 建立内部 CA：
  - `ClusterIssuer`: `local-ca-issuer`
  - CA 私钥：SOPS 加密存入 `infra/sops/secrets/`，setup 解密后 apply
...
- [ ] D3. 仓库初始化与幂等（强制工程化）：
  ...
  - 产物（infra）：`platform/nexus/scripts/`（groovy） + `scripts/nexus-init.sh`（调用 REST API 上传并执行脚本）
...
- [ ] F2. Pipeline 设计（两条，严格区分环境/构建形态）：
  ...
  - 产物：`infra/platform/tekton/pipelines/*.yaml`
...
- [ ] G4. Argo CD App 定义（已选 ApplicationSet）：
  ...
  - 使用 `ApplicationSet` 或单独 `Application` 管理 infra 中的平台组件（建议也用 ApplicationSet，按目录生成）
...
- [ ] I3. `setup.sh install`（通过 Argo CD 接管平台与应用）
  - 配置 Argo CD repo credentials（指向 infra 与 apps-deploy）
...
- 仓库模型：两仓库 GitOps（infra + apps-deploy）
